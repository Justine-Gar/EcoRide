...
ALTER TABLE `carpools`
  ADD CONSTRAINT fk_carpools_users FOREIGN KEY (id_user) REFERENCES users (id_user);

ALTER TABLE `cars`
  ADD CONSTRAINT fk_cars_users FOREIGN KEY (id_user) REFERENCES users (id_user) ON DELETE CASCADE;

ALTER TABLE `reviews`
    ADD CONSTRAINT fk_reviews_sender FOREIGN KEY (id_sender) REFERENCES users (id_user) ON DELETE SET NULL,
    ADD CONSTRAINT fk_reviews_recipient FOREIGN KEY (id_recipient) REFERENCES users (id_user) ON DELETE SET NULL,
    ADD CONSTRAINT fk_reviews_carpool FOREIGN KEY (id_carpool) REFERENCES carpools (id_carpool) ON DELETE CASCADE,
    ADD CONSTRAINT fk_reviews_users FOREIGN KEY (id_user) REFERENCES users (id_user) ON DELETE SET NULL;

ALTER TABLE `preference_types`
    ADD CONSTRAINT fk_preference_types_users FOREIGN KEY (id_user) REFERENCES users (id_user) ON DELETE SET NULL;
